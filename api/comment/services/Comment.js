...
module.exports = {
  ...
  fetchByPost: (params) => {
    ...
    return new Promise((resolve, reject) => {
      Comment
        .find({ post: params.postId })
        .populate({ path: 'user', select: 'username' }).then((items) => {
          for (let i = 0; i < items.length; i++) {

            array.push({ content: items[i].content, user: items[i].user['username'], date: moment(items[i].updatedAt).fromNow(), code: 1 });
            resolve(array);
          }
        }).then((err) => {
          resolve({ "message": 'not found comand', code: 0 });
  ...
  add: async (values) => {
    ...
    return Comment.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  /**
   * Promise to search a/an comment.
   *
   * @return {Promise}
   */
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Comment.attributes).reduce((acc, curr) => {
      switch (Comment.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
